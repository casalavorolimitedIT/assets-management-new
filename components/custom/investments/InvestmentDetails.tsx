...
> = {
  ...
  // fallback aliases from the modal (kept for backwards compat)
  basic: {
    icon: Zap,
    gradient: "from-emerald-400 via-teal-500 to-cyan-600",
    badge: "bg-emerald-100 text-emerald-700",
...
const projectedReturn = (amount: number, tenor: string) => {
  const months = parseInt(tenor) || 6;
  const rate = 0.18 / 12;
  return amount * months * (1 + rate);
};
...
export default function InvestmentDetails() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        // Map investment_plan — handle both verification-form shape and modal shape
        const ip = compliance?.investment_plan;
        if (ip) {
          // Helper to safely access dynamic properties
          const getDynamicProp = (obj: typeof ip, key: string): unknown => {
            return (obj as unknown as Record<string, unknown>)[key];
          };

          // Normalise: the verification form uses amount_figures for premium_plus / reif
          const monthly_amount_figures =
            (ip.monthly_amount_figures as number) ??
            (getDynamicProp(ip, "amount_figures") as number) ??
            (getDynamicProp(ip, "total_figures") as number) ??
            0;
          ...
          setPlan({
            ...
            monthly_amount_words:
              ...
              (getDynamicProp(ip, "amount_words") as string) ??
              (getDynamicProp(ip, "total_words") as string) ??
              "",
  ...
  const months = plan ? parseInt(plan.tenor) || 6 : 0;
    ...
    <>
      {showModal && (
        <AddPlanModal
          ...
          onSuccess={(newPlan) => setPlan(newPlan)}
        />
      )}
      ...
      <div className="min-h-screen bg-zinc-50/50">
        ...
        {plan ? (
          <div
            className={`relative mb-6 overflow-hidden rounded-2xl bg-linear-to-br ${meta.gradient} p-6 text-white shadow-lg`}
          >
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                ...
                <div className="rounded-xl bg-white/15 p-3 backdrop-blur-sm">
                  ...
                  <p className="text-sm font-bold">
                    {new Date(plan.monthly_payment_date).getDate()}th monthly
                  </p>
                </div>
                      ...
                      ? fmtDate(
                          new Date(
                            new Date(plan.monthly_payment_date).setMonth(
                              new Date(plan.monthly_payment_date).getMonth() +
                                months,
                            ),
                          )
                            .toISOString()
